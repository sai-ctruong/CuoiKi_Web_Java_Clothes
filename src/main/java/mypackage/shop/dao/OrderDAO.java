...
public class OrderDAO extends GenericDAO<Order> {
                ...
                for (OrderDetail detail : order.getOrderDetails()) {
                    if (detail.getProduct() != null) {
                        try {
                            ...
                            System.err.println("Warning: Could not load images for product " + detail.getProduct().getId() + ": " + e.getMessage());
                        }
                    }
                }
            }
    ...
    /**
     * Search orders by customer full name
     */
    public List<Order> searchOrdersByCustomerName(String keyword) {
        EntityManager em = getEntityManager();
        try {
            System.out.println("=== SEARCHING ORDERS BY NAME: " + keyword + " ===");
            String jpql = "SELECT o FROM Order o WHERE LOWER(o.fullName) LIKE LOWER(:keyword) ORDER BY o.orderDate DESC";
            List<Order> results = em.createQuery(jpql, Order.class)
